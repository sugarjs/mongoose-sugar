...
function get(model, id, fields, cb) {
    if(is.fn(fields)) model.findById(id, fields);
    else if(is.array(fields)) model.find({_id: id}, fields.join(' '), function(err, d) {
        cb(err, d && d[0]);
    });
    else cb('mongoose-sugar: invalid fields parameter!');
}
exports.get = get;
